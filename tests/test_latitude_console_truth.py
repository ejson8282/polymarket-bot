...
def test_account_ops_exposes_onboarding_deadlines_and_capital(monkeypatch) -> None:
    ...
    assert onboarding["ledger_entries"][0]["category"] == "开户成本"
    assert onboarding["ledger_cost"] == 88
    assert onboarding["actual_rewards"] == 900
    assert onboarding["net_income"] == 812
    assert onboarding["ipo_adjustment"] == -88
    assert onboarding["people_summary"][0]["net"] == 812
...
def test_console_contains_onboarding_and_reward_workflow() -> None:
    ...
    assert onboarding_section.index('class="onb-head"') < onboarding_section.index('class="ledger-summary"')
    assert 'data-onb-view="rewards"' in html
    assert 'data-onb-view="profiles"' in html
    assert 'data-onb-view="templates"' in html
    assert 'data-onb-view="ledger"' in html
    assert "资金任务链" not in onboarding_section
    assert "众安银行 → 平安银行 → 云锋有鱼 → 艾德证券" in html
    assert 'id="onb-show-settled"' in html
    assert "奖励已到账" in html
    assert "一键安排未加入项目" in html
    assert "收益与成本流水" in html
    assert "计入港股打新" in html
    ...
    assert "upsert_ledger_entry" in html
    assert "set_reward_received" in html
