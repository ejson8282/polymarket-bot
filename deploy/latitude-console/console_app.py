...
DEWU_EXECUTOR_STATUS_URL = f"{DEWU_EXECUTOR_URL}/status"
DEWU_EXECUTOR_SNAPSHOT_PATH = Path(
    os.getenv("DEWU_EXECUTOR_SNAPSHOT_PATH", DATA_DIR / "dewu_executor_last_good.json")
)
DEWU_LISTING_REVIEW_PATH = Path(
    os.getenv("DEWU_LISTING_REVIEW_PATH", DATA_DIR / "dewu_listing_reviews.json")
)
DEWU_LISTING_FAILURE_PATH = Path(
    os.getenv("DEWU_LISTING_FAILURE_PATH", DATA_DIR / "dewu_listing_failures.json")
)
API_STATE_SNAPSHOT_PATH = Path(
    os.getenv("API_STATE_SNAPSHOT_PATH", DATA_DIR / "console_state_last_good.json")
)
API_STATE_CACHE_TTL = float(os.getenv("API_STATE_CACHE_TTL", "12"))
_DEWU_EXECUTOR_LAST_GOOD: dict[str, Any] = {}
...
def _dewu_review_key(sku: Any, size: Any) -> str:
    return f"{str(sku or '').strip().upper()}|{str(size or '').strip().upper()}"


def _dewu_reviews_load() -> dict[str, dict]:
    payload = _read_json(DEWU_LISTING_REVIEW_PATH)
    reviews = payload.get("reviews") if isinstance(payload, dict) else None
    return reviews if isinstance(reviews, dict) else {}


def _dewu_reviews_save(reviews: dict[str, dict]) -> None:
    DEWU_LISTING_REVIEW_PATH.parent.mkdir(parents=True, exist_ok=True)
    tmp = DEWU_LISTING_REVIEW_PATH.with_suffix(".json.tmp")
    tmp.write_text(
        json.dumps({"version": 1, "reviews": reviews}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    os.replace(tmp, DEWU_LISTING_REVIEW_PATH)


def _dewu_failures_load() -> dict[str, dict]:
    payload = _read_json(DEWU_LISTING_FAILURE_PATH)
    failures = payload.get("failures") if isinstance(payload, dict) else None
    return failures if isinstance(failures, dict) else {}


def _dewu_failures_save(failures: dict[str, dict]) -> None:
    DEWU_LISTING_FAILURE_PATH.parent.mkdir(parents=True, exist_ok=True)
    tmp = DEWU_LISTING_FAILURE_PATH.with_suffix(".json.tmp")
    tmp.write_text(
        json.dumps({"version": 1, "failures": failures}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    os.replace(tmp, DEWU_LISTING_FAILURE_PATH)


def _dewu_listing_reason(detail: Any) -> str:
    """Turn worker traces into a short decision the operator can act on."""
    text = str(detail or "").strip()
    if "服饰颜色未能" in text or "配色" in text:
        return "需要确认得物商品颜色后再试"
    if "高可信商品卡" in text or "名称搜索" in text or "搜索未返回" in text:
        return "需要确认得物可搜索到的商品名称"
    if "暂无最低出售价" in text:
        return "当前没有最低价；可填写手动价格，或暂不出售"
    if "页面没有进入预期" in text or "timeout" in text.lower():
        return "手机页面跳转异常；可单独重试"
    for line in reversed(text.splitlines()):
        if "ListingWorkflowError:" in line:
            return line.split("ListingWorkflowError:", 1)[-1].strip()[:160]
    return "本规格未完成，需要确认后单独重试"


def _dewu_listing_item(item: dict, review: Optional[dict] = None) -> dict:
    review = review or {}
    name = str(review.get("name_override") or item.get("name") or "").strip()
    color = str(review.get("color") or item.get("color") or "").strip()
    # The Mac worker searches apparel by full title.  Appending the confirmed
    # color makes its deterministic colour-selector use the same variant.
    search_name = name
    if color and color.casefold() not in search_name.casefold():
        search_name = f"{search_name} {color}".strip()
    result = {
        "sku": item.get("sku"),
        "name": search_name,
        "color": color,
    ...
    if review.get("price_cny") is not None:
        result["price_cny"] = review["price_cny"]
    return result


def _persist_dewu_executor_snapshot(data: dict) -> None:
    """Keep a local good status so a dashboard refresh never waits on ADB/Tailnet."""
    try:
        DEWU_EXECUTOR_SNAPSHOT_PATH.parent.mkdir(parents=True, exist_ok=True)
        tmp = DEWU_EXECUTOR_SNAPSHOT_PATH.with_suffix(".json.tmp")
        tmp.write_text(json.dumps(data, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
        os.replace(tmp, DEWU_EXECUTOR_SNAPSHOT_PATH)
    except OSError:
        pass


def _dewu_executor(path: str, payload: Optional[dict] = None, timeout: float = 25.0) -> tuple[dict, int]:
...
def _dewu_executor_status(force_refresh: bool = False) -> tuple[dict, int]:
    """Return the warm status cache immediately; refresh happens in the background."""
    global _DEWU_EXECUTOR_LAST_GOOD
    if force_refresh:
        result, code = _dewu_executor("/status", timeout=3.0)
        if code < 400 and isinstance(result, dict) and result.get("ok"):
            _DEWU_EXECUTOR_LAST_GOOD = dict(result)
            _HTTP_CACHE[DEWU_EXECUTOR_STATUS_URL] = (dict(result), time.time())
            _persist_dewu_executor_snapshot(result)
        return result, code
    cached = _HTTP_CACHE.get(DEWU_EXECUTOR_STATUS_URL)
    if cached is not None and isinstance(cached[0], dict):
        return dict(cached[0]), 200

    snapshot = _read_json(DEWU_EXECUTOR_SNAPSHOT_PATH)
    if isinstance(snapshot, dict) and snapshot.get("ok"):
        _DEWU_EXECUTOR_LAST_GOOD = dict(snapshot)
        _HTTP_CACHE[DEWU_EXECUTOR_STATUS_URL] = (dict(snapshot), time.time())
        return dict(snapshot), 200

    # First-ever request only: cap the synchronous probe tightly.  Later page
    # reads always return a background-refreshed snapshot instead.
    result, code = _dewu_executor("/status", timeout=2.0)
    if code < 400 and isinstance(result, dict) and result.get("ok"):
        _DEWU_EXECUTOR_LAST_GOOD = dict(result)
        _HTTP_CACHE[DEWU_EXECUTOR_STATUS_URL] = (dict(result), time.time())
        _persist_dewu_executor_snapshot(result)
        return result, code
    if _DEWU_EXECUTOR_LAST_GOOD:
        stale = dict(_DEWU_EXECUTOR_LAST_GOOD)
        stale["stale"] = True
        return stale, 200
    return result, code


def _dewu_invalidate_status_cache() -> None:
    """An operator-triggered job needs a fresh status on its next read."""
    _HTTP_CACHE.pop(DEWU_EXECUTOR_STATUS_URL, None)
...
def dewu_inventory_get() -> JSONResponse:
    state = _dewu_load()
    reviews = _dewu_reviews_load()
    # Keep the Excel record immutable, but expose the operator's most recent
    # review decision so the inventory table immediately explains why a row
    # is not in the normal pending queue.
    items: list[dict] = []
    for source in state["items"]:
        item = dict(source)
        review = reviews.get(_dewu_review_key(item.get("sku"), item.get("size")), {})
        if review.get("decision") == "skip":
            item["review_status"] = "暂不出售"
        elif review.get("decision") in {"queued", "retry"}:
            item["review_status"] = "已加入确认队列"
        items.append(item)
    # Only malformed Excel rows belong in the active exception queue.  Earlier
    # manual corrections were historically stored beside them; expose those as
    # processed inventory events instead of falsely alarming the operator.
    active_exceptions = [
        row for row in state["exceptions"]
    ...
    events = [
        ...
        row for row in state["exceptions"]
        if isinstance(row, dict) and not isinstance(row.get("row"), int)
    ]
    ...
    return JSONResponse({
        **state,
        ...
        "spec_count": len(state["items"]),
    })


def _dewu_overview() -> dict:
    """Small, bounded summary for the console home page."""
    state = _dewu_load()
    items = state.get("items") or []
    total_quantity = sum(int(item.get("quantity") or 0) for item in items if isinstance(item, dict))
    import_exceptions = sum(
        1 for row in state.get("exceptions") or []
        if isinstance(row, dict) and isinstance(row.get("row"), int)
    )
    executor, _ = _dewu_executor_status()
    listed_quantity = int(executor.get("listed_quantity") or 0)
    listed_specs = int(executor.get("listed_spec_count") or 0)
    repricing = executor.get("repricing") if isinstance(executor.get("repricing"), dict) else {}
    results = repricing.get("results") if isinstance(repricing.get("results"), list) else []
    review_count = sum(
        1 for item in results
        if isinstance(item, dict) and item.get("action") in {"review", "skip"}
    )
    return {
        "present": bool(items) or bool(executor),
        "total_quantity": total_quantity,
        "spec_count": len(items),
        "listed_quantity": min(total_quantity, listed_quantity),
        "listed_spec_count": listed_specs,
        "pending_quantity": max(0, total_quantity - listed_quantity),
        "import_exceptions": import_exceptions,
        "executor_online": bool(executor.get("ok")),
        "device_connected": executor.get("device_connected"),
        "can_submit": executor.get("can_submit"),
        "repricing_state": repricing.get("state") or "not_run",
        "repricing_finished_at": repricing.get("finished_at") or repricing.get("at"),
        "repricing_review_count": review_count,
    }


@app.get("/api/dewu/executor")
def dewu_executor_get(fresh: bool = False) -> JSONResponse:
    result, code = _dewu_executor_status(force_refresh=fresh)
    return JSONResponse(result, status_code=code)


@app.post("/api/dewu/repricing/start")
async def dewu_repricing_start(request: Request) -> JSONResponse:
    try:
        payload = await request.json()
    except Exception:
        payload = {}
    if payload.get("confirmation") != "RUN_REPRICING":
        return JSONResponse({"ok": False, "error": "请确认运行一次改价"}, status_code=400)
    result, code = _dewu_executor("/jobs/reprice", payload)
    if code < 400:
        _dewu_invalidate_status_cache()
    return JSONResponse(result, status_code=code)


@app.post("/api/dewu/stocktake/start")
async def dewu_stocktake_start(request: Request) -> JSONResponse:
    try:
        payload = await request.json()
    except Exception:
        payload = {}
    if payload.get("confirmation") != "RUN_STOCKTAKE":
        return JSONResponse({"ok": False, "error": "请确认开始盘点"}, status_code=400)
    result, code = _dewu_executor("/jobs/stocktake", payload)
    if code < 400:
        _dewu_invalidate_status_cache()
    return JSONResponse(result, status_code=code)
...
async def dewu_listings_start(request: Request) -> JSONResponse:
    ...
    reviews = _dewu_reviews_load()
    items = [
        _dewu_listing_item(item, reviews.get(_dewu_review_key(item.get("sku"), item.get("size"))))
        for item in state["items"]
        if int(item.get("quantity") or 0) > 0
        and str(item.get("status") or "待处理") not in {"已上架", "已售出"}
        and reviews.get(_dewu_review_key(item.get("sku"), item.get("size")), {}).get("decision") != "skip"
    ]
...
@app.get("/api/dewu/listing/reviews")
def dewu_listing_reviews_get() -> JSONResponse:
    """Latest unresolved listing failures, enriched with operator overrides."""
    state = _dewu_load()
    reviews = _dewu_reviews_load()
    failures = _dewu_failures_load()
    inventory = {
        _dewu_review_key(item.get("sku"), item.get("size")): item
        for item in state.get("items", []) if isinstance(item, dict)
    }
    executor, _ = _dewu_executor_status()
    job = executor.get("job") if isinstance(executor.get("job"), dict) else {}
    results = job.get("results") if isinstance(job.get("results"), list) else []
    # Executor jobs are intentionally in-memory.  Persist only unresolved
    # failures here so starting a one-item retry never makes the other review
    # items disappear from the dashboard.
    changed = False
    for result in results:
        if not isinstance(result, dict):
            continue
        key = _dewu_review_key(result.get("sku"), result.get("size"))
        if result.get("ok"):
            if key in failures:
        ...
        candidate = {
            "sku": result.get("sku"), "size": result.get("size"),
            "quantity": result.get("quantity"), "detail": result.get("detail"),
            "at": previous.get("at") or datetime.now(timezone.utc).isoformat(),
        }
        if failures.get(key) != candidate:
            failures[key] = candidate
            changed = True
    if changed:
        _dewu_failures_save(failures)
    queue: list[dict] = []
    for key, result in reversed(list(failures.items())):
        if not isinstance(result, dict):
            continue
        ...
        queue.append({
            "key": key,
            "sku": result.get("sku") or item.get("sku") or "",
            "size": result.get("size") or item.get("size") or "",
            "name": item.get("name") or "",
            "color": item.get("color") or "",
            "quantity": item.get("quantity") or result.get("quantity") or 0,
            "reason": _dewu_listing_reason(result.get("detail")),
            "decision": saved.get("decision"),
            "name_override": saved.get("name_override") or "",
            "color_override": saved.get("color") or "",
            "price_cny": saved.get("price_cny"),
            "updated_at": saved.get("updated_at"),
        })
    return JSONResponse({"ok": True, "items": queue, "job_state": job.get("state")})


@app.post("/api/dewu/listing/reviews")
async def dewu_listing_reviews_submit(request: Request) -> JSONResponse:
    try:
        payload = await request.json()
    except Exception:
        payload = {}
    sku, size = str(payload.get("sku") or "").strip(), str(payload.get("size") or "").strip()
    action = str(payload.get("action") or "").strip()
    key = _dewu_review_key(sku, size)
    state = _dewu_load()
    item = next((row for row in state.get("items", [])
                 if isinstance(row, dict) and _dewu_review_key(row.get("sku"), row.get("size")) == key), None)
    if not sku or not size or item is None:
        return JSONResponse({"ok": False, "error": "找不到对应库存规格"}, status_code=404)
    if action not in {"retry", "queue", "skip"}:
        return JSONResponse({"ok": False, "error": "无效的确认操作"}, status_code=400)
    price = payload.get("price_cny")
    try:
        price = int(price) if str(price or "").strip() else None
        if price is not None and price < 1:
            raise ValueError
    except (TypeError, ValueError):
        return JSONResponse({"ok": False, "error": "手动价格必须是正整数"}, status_code=400)
    reviews = _dewu_reviews_load()
    review = {
        "decision": "skip" if action == "skip" else ("queued" if action == "queue" else "retry"),
        "name_override": str(payload.get("name_override") or "").strip()[:200],
        "color": str(payload.get("color") or "").strip()[:80],
        "price_cny": price,
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    reviews[key] = review
    _dewu_reviews_save(reviews)
    if action == "skip":
        return JSONResponse({"ok": True, "message": "已标为暂不出售；后续批量上架会跳过此规格"})
    if action == "queue":
        return JSONResponse({"ok": True, "message": "已加入确认队列；点击“开始确认队列”后统一执行"})
    result, code = _dewu_executor(
        "/jobs/listings",
        {"confirmation": "START_REAL_LISTING", "items": [_dewu_listing_item(item, review)]},
    )
    if code < 400:
        _dewu_invalidate_status_cache()
    return JSONResponse(result, status_code=code)
...
@app.post("/api/dewu/listing/reviews/run")
async def dewu_listing_reviews_run(request: Request) -> JSONResponse:
    """Run all operator-confirmed review items as one sequential phone job."""
    try:
        payload = await request.json()
    except Exception:
        payload = {}
    if payload.get("confirmation") != "START_REVIEW_QUEUE":
        return JSONResponse({"ok": False, "error": "请确认开始确认队列"}, status_code=400)
    state = _dewu_load()
    reviews = _dewu_reviews_load()
    items = [
        _dewu_listing_item(item, reviews[_dewu_review_key(item.get("sku"), item.get("size"))])
        for item in state.get("items", [])
        if isinstance(item, dict)
        and int(item.get("quantity") or 0) > 0
        and reviews.get(_dewu_review_key(item.get("sku"), item.get("size")), {}).get("decision") == "queued"
    ]
    if not items:
        return JSONResponse({"ok": False, "error": "没有已确认、待执行的规格"}, status_code=400)
    result, code = _dewu_executor(
        "/jobs/listings",
        {"confirmation": "START_REAL_LISTING", "items": items},
    )
    if code < 400:
        _dewu_invalidate_status_cache()
    return JSONResponse(result, status_code=code)
...
def _store_http_cache(url: str, data: Optional[dict]) -> None:
    ...
    if url == DEWU_EXECUTOR_STATUS_URL and isinstance(data, dict) and data.get("ok"):
        _DEWU_EXECUTOR_LAST_GOOD = dict(data)
        _persist_dewu_executor_snapshot(data)
...
def _account_ops() -> Dict[str, Any]:
    ...
    onboarding_ledger_entries = [
        item
        for item in onboarding_state.get(
            "ledgerEntries", onboarding_state.get("ledger_entries", [])
        )
        if isinstance(item, dict)
    ]
    ...
    normalized_onboarding_ledger = []
    for item in onboarding_ledger_entries:
        amount = _num(item.get("amount"))
        if amount is None:
            continue
        direction = str(item.get("direction") or "income").strip().lower()
        if direction not in {"income", "cost"}:
            direction = "income"
        normalized_onboarding_ledger.append(
            {
                "id": str(item.get("id") or ""),
                "person": str(item.get("person") or ""),
                "account_id": str(item.get("accountId") or ""),
                ...
                "count_in_ipo": bool(item.get("countInIpo")),
                "notes": str(item.get("notes") or ""),
                "updated_at": str(item.get("updatedAt") or ""),
            }
        )
    ...
    actual_rewards = round(
        sum(item["actual_reward"] or 0.0 for item in normalized_onboarding), 2
    )
    ledger_income = round(
        sum(
            item["amount"]
            for item in normalized_onboarding_ledger
    ...
    ipo_ledger_adjustment = round(
        sum(
            item["amount"] if item["direction"] == "income" else -item["amount"]
            for item in normalized_onboarding_ledger
            if item["count_in_ipo"]
        ),
        2,
    )
    onboarding_people_summary: Dict[str, Dict[str, Any]] = {}
    for item in normalized_onboarding:
        person = item["person"] or "未分配"
        summary = onboarding_people_summary.setdefault(
            person,
            {"person": person, "reward_income": 0.0, "other_income": 0.0, "cost": 0.0},
        )
        summary["reward_income"] += item["actual_reward"] or 0.0
    for item in normalized_onboarding_ledger:
        person = item["person"] or "未分配"
        summary = onboarding_people_summary.setdefault(
            person,
        ...
        key = "other_income" if item["direction"] == "income" else "cost"
        summary[key] += item["amount"]
    for summary in onboarding_people_summary.values():
        summary["net"] = round(
            summary["reward_income"] + summary["other_income"] - summary["cost"], 2
        )
        for key in ("reward_income", "other_income", "cost"):
    ...
    onboarding = {
        ...
        "ledger_entries": normalized_onboarding_ledger,
        "ledger_income": ledger_income,
        "ledger_cost": ledger_cost,
        "actual_rewards": actual_rewards,
        "net_income": round(actual_rewards + ledger_income - ledger_cost, 2),
        "ipo_adjustment": ipo_ledger_adjustment,
        "people_summary": sorted(
            onboarding_people_summary.values(),
            key=lambda item: item["person"],
        ),
    ...
    return {
        ...
        "income": round(income + ipo_ledger_adjustment, 2),
        "roi_pct": (
            round((income + ipo_ledger_adjustment) / capital * 100, 2)
            if capital
            else None
        ),
        "wear": round(wear, 2),
...
def _build_api_state() -> dict:
    """Build a complete console state; this may touch slow read-only sources."""
    pm = _polymarket()
...
_API_STATE_CACHE: Optional[dict] = None
_API_STATE_CACHE_AT = 0.0
_API_STATE_REFRESHING = False
_API_STATE_LOCK = threading.Lock()


def _persist_api_state_snapshot(data: dict) -> None:
    try:
        API_STATE_SNAPSHOT_PATH.parent.mkdir(parents=True, exist_ok=True)
        tmp = API_STATE_SNAPSHOT_PATH.with_suffix(".json.tmp")
        tmp.write_text(json.dumps(data, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
        os.replace(tmp, API_STATE_SNAPSHOT_PATH)
    except OSError:
        pass


def _refresh_api_state_cache() -> None:
    global _API_STATE_CACHE, _API_STATE_CACHE_AT, _API_STATE_REFRESHING
    try:
        data = _build_api_state()
        with _API_STATE_LOCK:
            _API_STATE_CACHE = data
            _API_STATE_CACHE_AT = time.time()
        _persist_api_state_snapshot(data)
    except Exception:
        # Keep the previous complete snapshot visible if one optional source
        # misbehaves; a monitoring page must not turn slow because of it.
        pass
    finally:
        with _API_STATE_LOCK:
            _API_STATE_REFRESHING = False
...
def _schedule_api_state_refresh() -> None:
    global _API_STATE_REFRESHING
    with _API_STATE_LOCK:
        if _API_STATE_REFRESHING:
            return
        _API_STATE_REFRESHING = True
    threading.Thread(target=_refresh_api_state_cache, name="latitude-state-refresh", daemon=True).start()


def _cached_api_state() -> dict:
    """Serve a warm or persisted page snapshot; refresh it without blocking HTTP."""
    global _API_STATE_CACHE, _API_STATE_CACHE_AT
    with _API_STATE_LOCK:
        if _API_STATE_CACHE is None:
            saved = _read_json(API_STATE_SNAPSHOT_PATH)
            if isinstance(saved, dict):
                _API_STATE_CACHE = saved
                _API_STATE_CACHE_AT = time.time()
        cached = _API_STATE_CACHE
        age = time.time() - _API_STATE_CACHE_AT if cached is not None else None
    if cached is None:
        # There is no prior snapshot only on the very first boot.  Build once,
        # then every later request is immediate while refresh runs in background.
        _refresh_api_state_cache()
        with _API_STATE_LOCK:
            cached = _API_STATE_CACHE or {}
    elif age is not None and age >= API_STATE_CACHE_TTL:
        _schedule_api_state_refresh()
    return dict(cached)


@app.get("/api/state")
def api_state() -> JSONResponse:
    return JSONResponse(_cached_api_state())
...
async def onboarding_action(payload: dict, request: Request) -> JSONResponse:
    ...
    detail = {
        ...
        "upsert_ledger_entry": "收益流水已保存",
        "delete_ledger_entry": "收益流水已删除",
    }[action]
    return JSONResponse({"ok": True, "msg": detail})
