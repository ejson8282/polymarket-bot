...
For later releases, replace `none` with the exact currently deployed Predict
SHA. Activation runs one real read-only/dry-run cycle and accepts the release
only when the state reports the exact SHA, `mode=dry_run`, one completed cycle,
and zero errors. A failure restores the previous Predict unit and release link.
