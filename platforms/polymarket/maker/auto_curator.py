...
class AutoCurator:
    ...
    async def _try_add_market(
        ...
        if session_label == "night":
            ...
            if seconds_to_start < MIN_TIME_TO_START_SEC:
                self._reject(yes_token_id, f"too_soon:{int(seconds_to_start/60)}min")
                return False
        ...
        if added:
            ...
            log(f"[auto_curator] ADDED session={session_label} token={yes_token_id[:16]} "
                ...
                f"reward=${daily_reward}/d game_in={game_in_h:.1f}h depth={detail}")
        return added
    ...
    async def _check_bid_depth(self, token_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            return False, f"book_err:{str(e)[:40]}"
        if not book:
