...
7. 所有改动必须使用 PR，禁止直接推送默认分支。范围明确、依赖已落地、无文件冲突且测试完成时
   直接创建 Ready PR；只有方案待裁决、依赖未落地、实现/测试未完成或高风险动作待授权时才使用
   Draft。PR 填写 owner、base/head、文件、测试和发布边界，生产发布时再补服务、回滚点和验收项。
